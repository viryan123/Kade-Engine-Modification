...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var bfCheck:String = 'bf';
		var dadCheck:String = 'dad';
		...
		if (!stageTesting)
		{
			...
			dad = new Character(100, 100, dadCheck);
			if (dad.frames == null)
				{
	...
	private function handleInput(evt:KeyboardEvent):Void { // this actually handles press inputs
		...
		if (dataNotes.length != 0)
		{
			...
			ana.hit = true;
			ana.hitJudge = Ratings.CalculateRating(coolNote, Math.floor((PlayStateChangeables.safeFrames / 60) * 1000));
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					if (gottaHitNote) {
						sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, daType, alt2);
					} else {
						sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, daType, alt);
					}
	...
	override public function update(elapsed:Float)
	{
		...
		if (!PlayStateChangeables.Optimize)
			if (FlxG.keys.justPressed.EIGHT && songStarted)
			{
				...
				FlxG.switchState(new StageDebugState(curStage, gf.curCharacter, boyfriend.curCharacter, dad.curCharacter));
				clean();
				FlxG.stage.removeEventListener(KeyboardEvent.KEY_DOWN, handleInput);
				FlxG.stage.removeEventListener(KeyboardEvent.KEY_UP, releaseInput);
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			var daRating:String = Ratings.CalculateRating(daNote, noteDiff); 
			...
			if (daRating == 'sick')
			{
				var tex:flixel.graphics.frames.FlxAtlasFrames = Paths.getSparrowAtlas('noteSplashesNew', 'shared');
				sploosh.frames = tex;
						...
						else if (possibleNotes.length > 0)
						{
							...
							for (coolNote in possibleNotes)
							{
								if (pressArray[coolNote.noteData])
								{
									...
									anas[coolNote.noteData].hitJudge = Ratings.CalculateRating(coolNote, Math.floor((PlayStateChangeables.safeFrames / 60) * 1000));
									anas[coolNote.noteData].nearestNote = [coolNote.strumTime,coolNote.noteData,coolNote.sustainLength];
									goodNoteHit(coolNote);
								}
							}
						}
						if (boyfriend.holdTimer > Conductor.stepCrochet * 0.0011 * boyfriend.holdLength && (!holdArray.contains(true) || PlayStateChangeables.botPlay))
							{
	...
	function noteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		{
			...
			note.rating = Ratings.CalculateRating(note, Math.floor((PlayStateChangeables.safeFrames / 60) * 1000));
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if(loadRep)
				{
					...
					note.rating = Ratings.CalculateRating(note);
